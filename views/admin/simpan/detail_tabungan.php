...
if (isset($_POST['search'])) {
	...
	<div class="table-responsive">
		<table class="display table table-bordered table-light" style="width: 70%;">
			...
			<tbody>
				...
				foreach ($bln as $k => $v) {
					$smp = mysqli_query($koneksi, "select month(tgl_simpan) AS bln, sum(jumlah_wajib) as wajib, sum(jumlah_sukarela) as sukarela, id_user, id_anggota from tb_simpanan where id_anggota = '$id_agt' and month(tgl_simpan) = '$k' group by month(tgl_simpan)");
			...
			<tfoot>
				<tr class="table-info">
					<?php 
					$jml = mysqli_query($koneksi, "SELECT SUM(jumlah_wajib) AS wajib, SUM(jumlah_sukarela) AS sukarela, id_user, id_anggota FROM tb_simpanan WHERE id_anggota = '$id_agt'");
